...
def detect_raio():
    ...
    # cora/photo   
    dataset = CoraDataset('./datasets', 'cora', added_new_nodes=0)

    # citeseer/pubmed
    # dataset = PlanetoidDataset('./datasets', 'pubmed', added_new_nodes=0)
    print("feature matrix is", dataset.data.x.size())

    gen_date = torch.load('./results/DAGPA/cora/added_node_60/train_percent_0.8/0.9/0/fea_mat.pt')


    # gen_date = torch.load('./results/DAGPA/pubmed/added_node_20/train_percent_0.7/0.9/0/fea_mat.pt')

    print("gen data is", gen_date.size())


    
    clf = COPOD()
    clf.fit(dataset.data.x)
    print(clf.predict(gen_date))
...
if __name__ == '__main__':
    ...
    #     print("GAN generated matrix is ", res.size())
