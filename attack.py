...
from attack.CEFGAT import GNNAttack
...
def build_args():
    ...
    parser.add_argument('--with_GAN', type=str2bool, default=True, help='using GAN for feature generation')
...
if __name__ == '__main__':
    ...
    # add feature mask generated by GAN
    cuda = True if torch.cuda.is_available() else False
    Tensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
    latent_dim = 128
    feature_generation_GAN = args.with_GAN

    # test feature sparsity effect, turn off GAN
    # feature_generation_GAN = False

    if feature_generation_GAN:
        ...
        with torch.no_grad():
            GAN_nodes = generator(Tensor(np.random.normal(0, 1, (added_node_num, latent_dim))).requires_grad_(False))
        res = GAN_nodes.detach().cpu()
        print("generated nodes is", res.size())
        print(res)
        feature_masks = GAN_nodes
    ...
    if attacker.mask_features:
        print("feature mask is", feature_masks[0], len(feature_masks[0]))
    ...
    feature_mat_file = f'{path}/{args.feat_sparsity}/{args.seed}/fea_mat.pt'
    if not args.with_GAN:
        file_path = f'{path}/{args.feat_sparsity}/{args.seed}'
        if not osp.isdir(file_path):
            os.makedirs(file_path)
        print("newly added feature matrix is", type(added_dataset.data.x[-added_node_num:]), "contenet is", added_dataset.data.x[-added_node_num:])
        torch.save(added_dataset.data.x[-added_node_num:], feature_mat_file)
